...
direction is the one that minimizes $E_{d}^{2}$, so it would
be selected by the search. Note that the error values $E_{d}$
...
filter that deemphasizes taps that differ too much from the pixel being filtered~\cite{Midtskogen2017}.
...
Yet~\cite{AWCY} online testing tool using the AV1 and Thor~\cite{Midtskogen2017} codecs, both
...
(8th August 2017) and \texttt{b5e5cc5}~\cite{Thor-git} (21st October 2017) for AV1 and Thor
respectively.
