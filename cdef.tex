...
Se start by describing the direction search. The search operates on the
...
the chroma damping shall also be $3$ (and not 2) because $\left\lfloor\log_2 S^{(s)}\right\rfloor=3$.
...
preset).  The filtering is applied one filter block at a time.  For each filter
block, the 0 - 3 bits are read to indicate the preset that will be
used for this filter block. The filter parameters are not coded for
filter blocks only containing skipped coding blocks, i.e. blocks
(whether intra or inter) without coded residuals.  Any individual
skipped coding blocks within a filter block have filtering disabled if
their skip condition bit is not set.
...
most types of video content, as shown in figure~\ref{fig:subjective}.
