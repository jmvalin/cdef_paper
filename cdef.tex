...
\begin{align}
y\left(i, j\right)=x(i, j) &+ \sum_{m,n \in R^{(p)}}{w}^{(p)}_{m,n}f(x\left(m, n) - x(i,j\right), S^{(p)}, D) \nonumber\\
                           &+ \sum_{m,n \in R^{(s)}}{w}^{(s)}_{m,n}f(x\left(m, n) - x(i,j\right), S^{(s)}, D)
...
\begin{equation}
D = \frac{\sigma_s^2 + \sigma_d^2 + C_1}{2\sqrt{\sigma_s^2\sigma_d^2 + C_2}}||\mathbf{s}-\mathbf{d}||^2\ ,
\end{equation}
