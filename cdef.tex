...
\usepackage{color}
\usepackage{url}
\usepackage[linkcolor=black,urlcolor=blue,citecolor=black,pdfpagemode=None,pdfstartview=FitH,pdfview=FitH,colorlinks=true,pdftitle=The Constrained Directional Enhancement Filter,pdfauthor=Jean-Marc Valin]{hyperref}

\makeatother

\begin{document}

\title{The Constrained Directional Enhancement Filter (CDEF)}
...
y\left(i, j\right)=&x(i, j) + \mathrm{round}\bigg( \nonumber\\
 & \sum_{m,n}{w}'_{d,m,n}f\left(x\left(m, n) - x(i,j\right), S', D\right) \nonumber\\
+& \sum_{m,n}{w}''_{d,m,n}f\left(x\left(m, n) - x(i,j\right), S'', D\right) \bigg)\ ,
\label{eq:linear-filter2}
\end{align}
where $S'$ and $S''$ and the strengths of the primary and secondary taps, respectively,
...
The strengths $S'$ and $S''$ and damping $D$ must be set high
enough to smooth out coding artifacts, but low enough to avoid
blurring details in the image.  For 8-bit content $S'$ ranges between $0$ and $15$, and $S''$ can be $0$, $1$,
...
becomes negative.  For instance, if for chroma $S'=15$ and the luma damping is $3$,
the chroma damping shall also be $3$ (and not 2) because $\left\lfloor\log_2 S'\right\rfloor=3$.

For higher bit depth (more than 8 bits), $S'$ and $S''$ are scaled according to
the extra bit depth, and $D$ is offset accordingly. For example, 12-bit
content can have $S'$ values of $0$, $16$, $32$, $...$, $240$, and
$D$ ranges from $7$ to $10$.  Picking the optimal
damping value is less critical for compression gains than picking the
optimal strengths.  $S'$ and $S''$ are chosen independently for
luma and chroma.


The signaled luma primary strength $S'$ is adjusted for each $8\times8$
block using the \textit{variance} value computed in algorithm~\ref{cap:direction-search}:
\begin{equation}
S'_{adj}=\left\{ \begin{array}{ll}
\left\lfloor\frac{S' \left(4 + \min\left(\left\lfloor \log_2\left\lfloor\frac{v}{2^{16}}\right\rfloor\right\rfloor, 12\right)\right) + 8}{16}\right\rfloor &, v \ge 2^{10}\\
0 &, \mathrm{otherwise}
...
skipped (i.e. have no coded residual).  Such skipped filter blocks have CDEF disabled.
...
some tools while keeping CDEF enabled may give a good complexity/compression trade-off.
...
artifacts from coded videos. By taking into account the
