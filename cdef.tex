...
\FOR{$d=0$ to $7$}
  \FOR{$i=0$ to $7$}
    \FOR{$j=0$ to $7$}
      ...
      \STATE $\mathrm{partial}[d][L] \leftarrow \mathrm{partial}[d][L] + \lfloor{\mathrm{pixel}[i][j] \cdot 2^{8 - \mathrm{bitdepth}}\rfloor - 128}$
...
\end{equation}
where the constraint function $f(d, S, D)$ operates on the difference between the filtered pixel and each of the
neighbour pixels. For small differences, $f\left(d, S, D\right)=d$, making the filter in
...
ramp-down controlled by $D$.  Fig.~\ref{fig:constraint-function-strength} illustrates
the effect of the strength on $f(\cdot)$ and Fig.~\ref{fig:constraint-function-damping}
illustrates the effect of damping. The function is symmetric for $d$ values around $0$.
...
\subsection{Primary and Secondary Taps}
...
edges or patterns. However, directional filtering alone is sometimes not sufficient to
...
reasons, CDEF defines \textit{primary} taps and \textit{secondary} taps.

\begin{align}
y\left(i, j\right)=&x(i, j) + \mathrm{round}\bigg( \nonumber\\
 & \sum_{m,n \in R^{(p)}}{w}^{(p)}_{m,n}f(x\left(m, n) - x(i,j\right), S^{(p)}, D) \nonumber\\
+& \sum_{m,n \in R^{(s)}}{w}^{(s)}_{m,n}f(x\left(m, n) - x(i,j\right), S^{(s)}, D) \bigg)\ ,
...
values. This is avoided by explicitly clamping the filter output based on $R^{(p)}$ and $R^{(s)}$:
\begin{align}
y_{clip}(i,j) &= \min\left(y_{max}, \max\left(y_{min}, y(i,j) \right) \right)\\
y_{min} &= \min_{m, n \in R^{(p)} \cup R^{(s)}}(x(i+m,j+n)) \\
y_{max} &= \max_{m, n \in R^{(p)} \cup R^{(s)}}(x(i+m,j+n))
\label{eq:cap-function}
\end{align}
Where $R$ is the set of pixels in the 5x5 neighbourhood of $x(i, j)$ with non-zero weights $w_{d,m,n}$.
...
Based on structural similarity (SSIM) metric~\cite{wang2004image}.
\begin{equation}
D_{8x8} = \frac{\sigma_s^2 + \sigma_d^2 + C_1}{2\sqrt{\sigma_s^2\sigma_d^2 + C_2}}\left\Vert\mathbf{s}-\mathbf{d}\right\Vert^2\ ,
\end{equation}
where $C_1=6.25$ and $C_2=312.5$ for 8-bit depth.
